...
### @chunk:hooks-mechanism | How Claude Code Hooks Work
### fr: Fonctionnement des hooks Claude Code
en: The hook system works in three steps: (1) `hooks/hooks.json` declares which events to listen to (SessionStart, PreToolUse, etc.) with a regex matcher, (2) when the event fires and matches, Claude Code executes the shell script specified in the `command` field, using `${CLAUDE_PLUGIN_ROOT}` as the plugin's base path, (3) the script outputs JSON to stdout with a `hookSpecificOutput` object containing `additionalContext` - this text gets injected into the conversation as a system reminder visible to Claude but seamless to the user.
fr: Le système de hooks fonctionne en trois étapes : (1) `hooks/hooks.json` déclare quels événements écouter (SessionStart, PreToolUse, etc.) avec un matcher regex, (2) quand l'événement se déclenche et correspond, Claude Code exécute le script shell spécifié dans le champ `command`, en utilisant `${CLAUDE_PLUGIN_ROOT}` comme chemin de base du plugin, (3) le script sort du JSON sur stdout avec un objet `hookSpecificOutput` contenant `additionalContext` - ce texte est injecté dans la conversation comme un rappel système visible par Claude mais transparent pour l'utilisateur.

> Decision: Used shell script output with JSON format because it allows dynamic content generation (reading skill files, checking project state) while maintaining a simple interface. The `additionalContext` field was chosen over `blockToolCall` to add information without interrupting the workflow.
> fr: Utilisé la sortie de script shell au format JSON car cela permet la génération de contenu dynamique (lecture des fichiers de skills, vérification de l'état du projet) tout en maintenant une interface simple. Le champ `additionalContext` a été choisi plutôt que `blockToolCall` pour ajouter des informations sans interrompre le workflow.

### @chunk:context-ingestion | Context Ingestion Mechanism
### fr: Mécanisme d'ingestion du contexte
en: The context ingestion is the non-obvious part of hooks. The script outputs raw JSON to stdout: `{"hookSpecificOutput": {"additionalContext": "your text"}}`. Claude Code captures this output, parses the JSON, and transforms `additionalContext` into a `<system-reminder>` XML tag that gets appended to the conversation. The user sees "SessionStart hook success" in the UI, but Claude receives the full text content wrapped in system-reminder tags, making it part of the conversation context without being a visible user message. This allows plugins to "whisper" instructions to Claude that guide its behavior throughout the session.
fr: L'ingestion du contexte est la partie non évidente des hooks. Le script sort du JSON brut sur stdout : `{"hookSpecificOutput": {"additionalContext": "votre texte"}}`. Claude Code capture cette sortie, parse le JSON, et transforme `additionalContext` en une balise XML `<system-reminder>` qui est ajoutée à la conversation. L'utilisateur voit "SessionStart hook success" dans l'UI, mais Claude reçoit le contenu textuel complet enveloppé dans des balises system-reminder, en faisant partie du contexte de conversation sans être un message utilisateur visible. Cela permet aux plugins de "chuchoter" des instructions à Claude qui guident son comportement tout au long de la session.

> Decision: The script must output valid JSON and handle special characters (newlines, quotes, backslashes) by escaping them properly. We use a bash function `escape_for_json()` that iterates character by character because standard tools like `jq` may not be available on all systems. Exit code 0 is required for success.
> fr: Le script doit sortir du JSON valide et gérer les caractères spéciaux (retours à la ligne, guillemets, backslashes) en les échappant correctement. On utilise une fonction bash `escape_for_json()` qui itère caractère par caractère car les outils standards comme `jq` peuvent ne pas être disponibles sur tous les systèmes. Le code de sortie 0 est requis pour le succès.
